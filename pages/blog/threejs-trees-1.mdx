...
The entire effect is built on the fact that we can translate vertices relative to the camera.
...
Resulting in the vertex moving one unit to the right relative to the camera.
If we were to subtract `1.0` it would translate one unit to the left and so on.

## Using UVs as offset values

Instead of hardcoding values for the translation we want to instead pass data to the shader,
and we can do this by abusing how UV maps work as each vertex has an assigned `[u, v]` numeric position.
