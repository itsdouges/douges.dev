...
import { NoAlphaTree } from 'components/examples/threejs-trees/no-alpha';
...
If it was the entire quad would always be in view when rotating around it with the camera,
instead it kind of bends when not looking at it directly.
...
Personally when I think of translating things in 3D space I think in local & world space so the mind blowing thing here is since GLSL shaders can move between coordinate spaces you can do things you wouldn't usually think of!
...
And then now we can perform some magic.
...
Instead of hardcoding values for the translation we need to pass data to the shader somehow.
...
So we're left with these four `[x, y]` values for each vertex:

1. Bottom left `[-1.0, -1.0]`
1. Top left `[1.0, -1.0]`
1. Top right `[1.0, 1.0]`
1. Bottom right `[1.0, -1.0]`

## Performing the translation

All that's left is to add the offset to the `worldViewPosition`,
we also pass the `vertexOffset` through the `normalize` function so the entire length of the vector equals `1`.

```glsl
worldViewPosition += vec4(normalize(vertexOffset), 0.0);
```

Now from whatever angle we're looking at the quad each vertex is translated across the `xy` axis resulting in a billboard-like effect where from the front it looks proportionally the same but bigger and from any other angle a stretched rectangle.
For foliage this works in our favour as it results in a subtler billboard effect.
