...
Now from whatever angle we're looking at the quad each vertex is translated across the `xy` axis resulting in a billboard-like effect where from the front it looks proportionally the same but bigger,
and from any other angle a stretched rectangle.
For foliage this works in our favour as it results in a subtler effect.
