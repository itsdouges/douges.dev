...
Unfortunately however this tutorial was made for Unity 3D not ThreeJS!
After learning up on GLSL shaders and a few days of [posting on Twitter](https://twitter.com/itsdouges/status/1587314407520235520) I've gotten something pretty nice,
take a look:

<TreeComplete />

Chuffed as for how it turned out,
it's really been a great learning experience.
Keep reading for how the effect is made,
there's a lot of opportunity here to learn something new!
...
The entire effect is built on the fact that we can move verticies relative to the camera.
...
Now we can move the vertex in view space.
...
We've now moved this vertex one unit to the right relative to the camera.
...
From any angle each vertex is now moved across the cameras `xy` axis resulting in a billboard-like effect where from the front it looks proportionally the same but bigger and from all other angles as a stretched rectangle.
...
Applying the rotation in view space so it is relative to the camera looks good.
