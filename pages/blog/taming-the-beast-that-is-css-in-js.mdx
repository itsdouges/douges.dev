...
almost impossible to successfully lint and evolve the written styles at scale.
...
and dynamic styles flow through the `style` prop.

## Code analysis

Imagine the scenario described earlier whereby styles are defined with no clear call site and nested conditional rest objects or if statements,
and then also imagine trying to write a lint rule,
it's no easy feat!
